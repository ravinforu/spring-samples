...
public class EndpointLoggingListener implements ApplicationEventListener {
    ...
    public EndpointLoggingListener() {
        this.applicationPath = "http://localhost:9090";
    }

    @Override
    public void onEvent(ApplicationEvent event) {
        if (ApplicationEvent.Type.INITIALIZATION_APP_FINISHED.equals(event.getType())) {
            final ResourceModel resourceModel = event.getResourceModel();
            final ResourceLogDetails logDetails = new ResourceLogDetails();
            resourceModel.getResources().stream().forEach((resource) -> logDetails.addEndpointLogLines(getLinesFromResource(resource)));
            logDetails.log();
        }
    }
    ...
    private void populate(String basePath, Class<?> klass,
                          Set<EndpointLogLine> endpointLogLines) {
        populate(basePath, true, Resource.from(klass), endpointLogLines);
    }
    ...
    private void populate(String basePath, boolean isLocator, Resource resource,
        ...
        for (ResourceMethod method : resource.getResourceMethods()) {
            ...
            endpointLogLines.add(new EndpointLogLine(method.getHttpMethod(), basePath));
        }
        ...
        for (Resource childResource : resource.getChildResources()) {
            for (ResourceMethod method : childResource.getAllMethods()) {
                if (method.getType() == ResourceMethod.JaxrsType.RESOURCE_METHOD) {
                    ...
                    endpointLogLines.add(new EndpointLogLine(method.getHttpMethod(), path));
                    ...
                    populate(path, erasedType, endpointLogLines);
                }
            }
        }
    }
    ...
    private static class ResourceLogDetails {
        public final static Logger logger = Logger.getLogger(ResourceLogDetails.class.getName());
    ...
    private static class EndpointLogLine {
        ...
        private EndpointLogLine(String httpMethod, String path) {
